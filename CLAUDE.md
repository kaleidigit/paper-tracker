...
| `src/config.ts` | loadProfileContext / applyDefaults / resolvePath | 无 |
...
```
profiles/{name}/
  config.json       ← 领域配置（app, pipeline, ai, feishu, sources）
...
2. **修改配置**：
   - `config.json` → 修改 `ai.prompts`（筛选/翻译/分类 prompt）
   - `journals.json` → 修改期刊列表
   - `classification.json` → 修改分类树
   - `feishu.notify_chat_id` → 修改飞书群 ID
...
```jsonc
"ai.filter.max_checks_per_run": 40   // LLM 过滤预算上限
"ai.filter.min_confidence": 0.5      // 过滤最低置信度
"ai.enrich.concurrency": 3           // 翻译分类并发数
"ai.translation.enabled": true       // 是否翻译
```
...
```bash
# 完整管道（串行 5 步，默认 profile=top-journal-env-energy）
./run.sh
./run.sh --dry-run
./run.sh --profile env-economics-journal
./run.sh --profile env-economics-journal --dry-run

# 自动推送（cron 入口，依次运行所有 profile，周一推3天其余推1天）
./auto-push.sh
...
配置文件 `config.json` 中**不需要**存储 shell 命令模板（如 `doc_publish_cmd` / `notify_cmd`），直接用 `--profile` 指定 profile 目录即可。
